...
public class ByteObjectHashMap<V> implements ByteObjectMap<V> {
    ...
    private static int hashCode(byte b2) {
        return b2;
    }

    private int hashIndex(byte b2) {
        return hashCode(b2) & this.mask;
    }

    private int indexOf(byte b2) {
        int hashIndex = hashIndex(b2);
        int i = hashIndex;
    ...
    public V put2(Byte b2, V v) {
        return put(objectToKey(b2), (byte) v);
    }
}

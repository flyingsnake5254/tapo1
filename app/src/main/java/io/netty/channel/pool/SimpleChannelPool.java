...
public class SimpleChannelPool implements ChannelPool {
    ...
    public void doHealthCheck(final Channel channel, final Promise<Channel> promise) {
        ...
        if (isHealthy.isDone()) {
            ...
            isHealthy.addListener(new a<Boolean>() { // from class: io.netty.channel.pool.SimpleChannelPool.4
    ...
    private void doHealthCheckOnRelease(final Channel channel, final Promise<Void> promise) throws Exception {
        ...
        if (isHealthy.isDone()) {
            ...
            isHealthy.addListener(new a<Boolean>() { // from class: io.netty.channel.pool.SimpleChannelPool.6
                @Override // io.netty.util.concurrent.GenericFutureListener
                public void operationComplete(Future<Boolean> future) throws Exception {
    ...
    public SimpleChannelPool(Bootstrap bootstrap, final ChannelPoolHandler channelPoolHandler, ChannelHealthChecker channelHealthChecker, boolean z, boolean z2) {
        ...
        clone.handler(new ChannelInitializer<Channel>() { // from class: io.netty.channel.pool.SimpleChannelPool.1
            static final /* synthetic */ boolean $assertionsDisabled = false;

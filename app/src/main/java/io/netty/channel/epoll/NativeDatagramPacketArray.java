    ...
    public final class NativeDatagramPacket {
        ...
        public DatagramPacket newDatagramPacket(ByteBuf byteBuf, InetSocketAddress inetSocketAddress) throws UnknownHostException {
            ...
            if (this.addrLen == NativeDatagramPacketArray.this.ipv4Bytes.length) {
                ...
                inetAddress = InetAddress.getByAddress(NativeDatagramPacketArray.this.ipv4Bytes);
            } else {
                inetAddress = Inet6Address.getByAddress((String) null, this.addr, this.scopeId);
            }
            return new DatagramPacket(byteBuf.writerIndex(this.count), inetSocketAddress, new InetSocketAddress(inetAddress, this.port));
        }
    }

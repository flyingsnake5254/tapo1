...
public abstract class HttpHeaders implements Iterable<Map.Entry<String, String>> {
    ...
    private static boolean containsCommaSeparatedTrimmed(CharSequence charSequence, CharSequence charSequence2, boolean z) {
        if (z) {
            int indexOf = AsciiString.indexOf(charSequence, StringUtil.COMMA, 0);
            if (indexOf != -1) {
                int i = 0;
                while (!AsciiString.contentEqualsIgnoreCase(AsciiString.trim(charSequence.subSequence(i, indexOf)), charSequence2)) {
                    i = indexOf + 1;
                    indexOf = AsciiString.indexOf(charSequence, StringUtil.COMMA, i);
                    if (indexOf == -1) {
                ...
                return true;
            } else if (AsciiString.contentEqualsIgnoreCase(AsciiString.trim(charSequence), charSequence2)) {
                return true;
            }
        } else {
            int indexOf2 = AsciiString.indexOf(charSequence, StringUtil.COMMA, 0);
            if (indexOf2 != -1) {
                int i2 = 0;
                while (!AsciiString.contentEquals(AsciiString.trim(charSequence.subSequence(i2, indexOf2)), charSequence2)) {
                    i2 = indexOf2 + 1;
                    indexOf2 = AsciiString.indexOf(charSequence, StringUtil.COMMA, i2);
                    if (indexOf2 == -1) {
                ...
                return true;
            } else if (AsciiString.contentEquals(AsciiString.trim(charSequence), charSequence2)) {
                return true;
            }
        }
        return false;

...
public class DefaultHttpDataFactory implements HttpDataFactory {
    ...
    public Attribute createAttribute(HttpRequest httpRequest, String str, String str2) {
        ...
        if (this.useDisk) {
            try {
                attribute = new DiskAttribute(str, str2, this.charset, this.baseDir, this.deleteOnExit);
                attribute.setMaxSize(this.maxSize);
            } catch (IOException unused) {
                attribute = new MixedAttribute(str, str2, this.minSize, this.charset, this.baseDir, this.deleteOnExit);
                attribute.setMaxSize(this.maxSize);
            }
            checkHttpDataSize(attribute);
            getList(httpRequest).add(attribute);
            return attribute;
        } else if (this.checkSize) {

...
public class SpdyFrameDecoder {
    ...
    private static boolean isValidFrameHeader(int i, int i2, byte b2, int i3) {
        switch (i2) {
            case 0:
                return i != 0;
            case 1:
            ...
            switch (AnonymousClass1.$SwitchMap$io$netty$handler$codec$spdy$SpdyFrameDecoder$State[this.state.ordinal()]) {
                case 1:
                    if (byteBuf.readableBytes() >= 8) {
                        ...
                        byteBuf.skipBytes(8);
                        if ((byteBuf.getByte(readerIndex) & DefaultBinaryMemcacheRequest.REQUEST_MAGIC_BYTE) == 0) {
                            z2 = false;
                        }
                        if (z2) {
                        ...
                        if (i == this.spdyVersion) {
                            if (isValidFrameHeader(this.streamId, i2, this.flags, unsignedMedium)) {
                ...
                case 3:
                    if (byteBuf.readableBytes() >= 10) {
                        ...
                        byte b2 = (byte) ((byteBuf.getByte(readerIndex2 + 8) >> 5) & 7);
                        ...
                        if (i7 != 0) {
                            ...
                            this.delegate.readSynStreamFrame(i7, unsignedInt, b2, hasFlag, hasFlag2);
                            break;
                        } else {
                            this.state = State.FRAME_ERROR;
                ...
                case 7:
                    if (this.numSettings != 0) {
                        if (byteBuf.readableBytes() >= 8) {
                            ...
                            boolean hasFlag5 = hasFlag(b3, (byte) 1);
                            boolean hasFlag6 = hasFlag(b3, (byte) 2);
                            byteBuf.skipBytes(8);
                            this.numSettings--;

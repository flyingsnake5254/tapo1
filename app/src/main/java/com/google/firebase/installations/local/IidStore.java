...
public class IidStore {
    private static final String[] ALLOWABLE_SCOPES = {WebSocketServerHandshaker.SUB_PROTOCOL_WILDCARD, FirebaseMessaging.INSTANCE_ID_SCOPE, CodePackage.GCM, ""};
    ...
    private static String getDefaultSenderId(FirebaseApp firebaseApp) {
        ...
        String[] split = applicationId.split(SSLClient.COLON);
        if (split.length != 4) {
            return null;
        }
        String str = split[1];
    ...
    private static String getIdFromPublicKey(@NonNull PublicKey publicKey) {
        try {
            ...
            digest[0] = (byte) (((digest[0] & BinaryMemcacheOpcodes.PREPEND) + 112) & 255);

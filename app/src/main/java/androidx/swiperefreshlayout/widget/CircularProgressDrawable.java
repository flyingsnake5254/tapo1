    ...
    void applyTransformation(float f2, Ring ring, boolean z) {
        ...
        if (this.mFinishing) {
            ...
            if (f2 < 0.5f) {
                ...
                f4 = (MATERIAL_INTERPOLATOR.getInterpolation(f2 / 0.5f) * 0.79f) + MIN_PROGRESS_ARC + f3;
            } else {
                float startingStartTrim = ring.getStartingStartTrim() + 0.79f;
                f3 = startingStartTrim - (((1.0f - MATERIAL_INTERPOLATOR.getInterpolation((f2 - 0.5f) / 0.5f)) * 0.79f) + MIN_PROGRESS_ARC);
                f4 = startingStartTrim;
            }
